# sample request curl for offchain sm definition:
```
postman request POST 'http://speedtest.param.network:8450/api/pipelines/pipe%3Asys%3Adefine-offchain-sm-v1/execute?dryRun=false' \
  --header 'Accept: */*' \
...
# sample response format for offchain sm definition:
```
{
    "status": "accepted",
    "data": {
...
# mongodb storage for offchain sm definition:
```
  {
