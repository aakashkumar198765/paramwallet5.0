...
apiClient.interceptors.request.use((config) => {
    ...
    if (paramId)
        config.headers['X-Param-ID'] = paramId;
    if (activeWorkspace)
        config.headers['X-Workspace'] = activeWorkspace;
    if (activeSuperApp)
        config.headers['X-SuperApp-ID'] = activeSuperApp._id;
    return config;
});
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        if (!refreshPromise) {
            refreshPromise = apiClient
                .post('/auth/refresh', { refreshToken, paramId })
                .then((res) => {
                setAuth({
