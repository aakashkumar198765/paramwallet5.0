...
const PIPELINE_ID = 'pipe:sys:define-offchain-sm-v1';
const POLL_INTERVAL_MS = 2500;
const TIMEOUT_MS = 60_000;

/**
 * Deploy an offchain state machine definition via ParamGateway.
 * Executes the pipeline then polls until all tasks are synced.
 */
export async function deployOffchainSM(payload: unknown): Promise<DeployResult> {
