...
const PIPELINE_ID = 'pipe:sys:define-onchain-sm-v1';
const POLL_INTERVAL_MS = 2500;
const TIMEOUT_MS = 60_000;

/**
 * Deploy an onchain state machine definition via ParamGateway.
 * Executes the pipeline then polls until all tasks are synced.
 */
export async function deployOnchainSM(payload: unknown): Promise<DeployResult> {
