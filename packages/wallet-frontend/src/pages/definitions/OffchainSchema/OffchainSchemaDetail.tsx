...
import { useOffchainSchema } from '@/hooks/useDefinitions';
...
export default function OffchainSchemaDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: schema, isLoading } = useOffchainSchema(id!);
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto p-6">
      <div className="mb-6 flex items-center gap-3">
        ...
        <Button onClick={() => navigate(`/definitions/offchain/schema/${id}/edit`)}>
